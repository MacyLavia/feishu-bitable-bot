 ...
 *
 * 用法：
 *   node update-usage-doc.js --full     全量重写文档（覆盖现有内容）
 *   node update-usage-doc.js --append   在文档末尾追加新变更章节（不修改原文）
 */

// 清除代理（与 bot-server.js 同理）
...
const MODE = process.argv.includes('--append') ? 'append'
           : process.argv.includes('--full')   ? 'full'
           : null;

if (!MODE) {
  console.log('用法:');
  console.log('  node update-usage-doc.js --full     全量重写文档');
  console.log('  node update-usage-doc.js --append   在末尾追加新变更章节');
  process.exit(0);
}
...
// ── 追加内容（--append 模式使用）────────────────────────────
// 每次迭代后在这里定义本次新增/变更的内容，追加到文档末尾
// 用户自行决定是否将内容合并回正文
const APPEND_BLOCKS = [

  hr(),
  h2('🔐 变更内容 · 2026-03-11 · 凭证安全加固'),
  p(run('将所有硬编码的 API 密钥迁移至 .env 文件管理，彻底清除 GitHub 历史中的明文凭证。')),

  h3('新增文件'),
  bl(mono('config.js'), run('  —  统一凭证加载器，解析本地 .env 文件并导出给所有脚本使用（无需 dotenv 依赖）')),
  bl(mono('.env'), run('  —  本地密钥文件（已加入 .gitignore，不提交 GitHub）')),
  bl(mono('.env.example'), run('  —  密钥模板文件（提交 GitHub），其他开发者参照填写自己的密钥')),

  h3('变更文件'),
  p(run('以下 6 个脚本移除硬编码密钥，改为 '), mono("require('./config')"), run(' 读取：')),
  ol(mono('bot-server.js')),
  ol(mono('ai-scoring.js')),
  ol(mono('patch-cases.js')),
  ol(mono('run-model-text-test.js')),
  ol(mono('run-media-test.js')),
  ol(mono('update-usage-doc.js')),

  h3('从 GitHub 移除'),
  bl(mono('delete-docs.js'), run('  —  一次性工具脚本，不面向其他开发者')),
  bl(mono('发送通知.bat'), run('  —  本地 Windows 路径硬编码，不通用')),

  h3('其他开发者使用方式'),
  p(run('克隆仓库后执行：')),
  code('cp .env.example .env\n# 编辑 .env，填入自己的飞书 APP_ID / APP_SECRET / SILICONFLOW_KEY\nnpm install\nnode bot-server.js'),
...
async function main() {
  ...
  await writeBlocks(token, BLOCKS, 0);
